	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < su.length -1; i++) {
			for (int j = i + 1; j < su.length; j++) {
				// 나(i)보다 남(j)이 작으면면 자리 변경( 오름차순 su[i] > su[j])
				// 나(i)보다 남(j)이 크면면 자리 변경 (내림 차순 su[i] <su[j]) 
				if(su [i] > su[j]) {
					tmp = su[j] ;
					su [j] = su[i];
					su [i] = tmp;
		...
		//Integer []나중에 
		// 클래스를 자료형 배열로 사용 (참조자료형 배열일떄 내림차순 가능)
		Integer[] su3 = {3,4,9,5,6,1,7,2,10,8};
		
		// Arrays.sort(배열, Collection.reverseOrder())
